  ...
  <form v-on:submit.prevent="onSubmit">
    <v-card>
      ...
      <v-card-text>
        ...
        <v-snackbar :timeout="timeout" :success="context === 'success'" :error="context === 'error'" v-model="snackbar">
          {{message}}
        </v-snackbar>
      </v-card-text>
      <v-card-actions>
        <v-btn v-if="!id" type="reset" flat ripple dark class="orange--text">Reset</v-btn>
        <v-btn type="submit" flat ripple dark class="green--text">Save User</v-btn>
      </v-card-actions>
    </v-card>
  </form>
</template>
